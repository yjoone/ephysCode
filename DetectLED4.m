function [LEDtriggers,LEDvideoNumMat,centroids] = DetectLED4(videoFilename,videoNum,varargin)
...
% output is the frame numbers when the flashing LED is on. 
...
% while i<=endFrame % edit for detectLED4
while hasFrame(voleObj)
    image = readFrame(voleObj);
    image = imcrop(image,imageCropParams);
    image = imcrop(image,boundBox);
