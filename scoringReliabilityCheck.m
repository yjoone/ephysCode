function [scorer1MatFrames,scorer2MatFrames,scorer1Mat,scorer2Mat] = ...
    scoringReliabilityCheck(filename_Scorer1,filename_Scorer2,behavIDName,behaviorDurationThreshold,filepath)
% This was adapted from 

if nargin < 4
    threshold = 0;
    filepath = 'R:\LiuLab\People\Jim\Experiments\OTmanipEphysExpt\VideoScoring';
elseif nargin <5
    filepath = 'R:\LiuLab\People\Jim\Experiments\OTmanipEphysExpt\VideoScoring';
end
...
iterfilename_Scorer1 = fullfile(filepath,filename_Scorer1);
iterfilename_Scorer2 = fullfile(filepath,filename_Scorer2);
...
% behaviorDurationThreshold = 1; % seconds. This serves as a minimum duration to compare the accuracy
...
for Scorer = 1:2
    ...
    shortBehaviorEpochesToBeRemoved = duration <= behaviorDurationThreshold;
...
set(gca,'yticklabel',{'Scorer 2 only' 'Scorer 1 only' 'Both scored'  'Scorer 2' 'Scorer 1'})
set(gca,'ytick',[.1:.2:.9])
